...
/**
  ...
  */
trait CrudLogicTestTrait[S<:Row, T<:Row, K<:Entity[T], L<:CrudLogic[S, T,K], P<:CrudPersistence[T,K]] extends PlaySpec with BeforeAndAfterEach with ScalaFutures with MockitoSugar with CrudTest {
  ...
  def getTest:Unit={
    "Al solicitar un objeto" must {
      "Se deberia retorna el objeto dado por la base de datos" in {
        ...
        when(persistenceMock.getAction(any())) thenReturn mock[DBIO[Option[T]]]
  ...
  def getAllTest:Unit={
    "Al solicitar multiples objetos" must {
      "La lista de objetos recibida debe ser igual a la dada por la base de datos" in {
        ...
        when(persistenceMock.getAllAction(any(), anyInt(),anyInt())) thenReturn mock[DBIO[Seq[T]]]
  ...
  def updateTest:Unit={
    "Al actualizar un objeto" must {
      "Se debe recibir el objeto actualizado en la base de datos" in {
        ...
        val id = Random.nextInt(20)
        when(persistenceMock.updateAction(anyInt(), any())) thenReturn mock[DBIO[Option[T]]]
  ...
  def deleteTest:Unit={
    "Al eliminar un objeto" must {
      "Se debe recibir el objeto eliminado en la base de datos" in {
        ...
        val id = Random.nextInt(20)
        when(persistenceMock.deleteAction(anyInt())) thenReturn mock[DBIO[Option[T]]]
