...
class BookController @Inject()(override val logic:BookLogicTrait, override val silhouette:Silhouette[MyEnv]) extends CrudController[Book, Books] with AuthController {
  override implicit val format = Json.format[Book]

  override def getAll = SecuredAction.async{implicit request =>
