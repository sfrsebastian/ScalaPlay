...
trait PersistenceTestTrait[T<:Row, K<:Entity[T]] extends PlaySpec with BeforeAndAfterEach with BeforeAndAfterAll with ScalaFutures{
  ...
  "Al solicitar multiples objetos" must {
    ...
    "Solicitar por consulta: Se debe recibir una lista de longitud 1 cuando se filtra por una consulta" in {
      val id = Random.nextInt(20)
      val libroExistente = seedCollection(id)
      val query = persistence.table.filter(_.name === libroExistente.name)
